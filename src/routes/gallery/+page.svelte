...
<svelte:head><title>Production asset gallery · Istanbul</title><meta name="description" content="Review every production asset used by Istanbul." /></svelte:head>
<svelte:window onkeydown={(event) => { if (selected && event.key === 'Escape') selected = null; }} />

{#snippet artwork(item: GalleryItem, large = false)}
  {#if item.kind === 'location'}<GameArt kind="location" place={item.place} class={large ? 'gallery-art gallery-art-large' : 'gallery-art'} label={item.title} />
  {:else if item.kind === 'mat'}<GameArt kind="mat" color={item.color} class={large ? 'gallery-art gallery-art-large' : 'gallery-art'} label={item.title} />
  ...
  {:else if item.kind === 'card'}<GameArt kind="card" effect={item.effect} class={large ? 'gallery-art gallery-art-large' : 'gallery-art'} label={item.title} />
  ...
  {:else}
    <span class:large class="demand-composition" aria-label={`${item.title}: ${item.detail}`} role="img">
      <GameArt kind="component" component={item.market === 'large' ? 'demand-large' : 'demand-small'} class="demand-base" />
      <span>{#each item.goods as good}<GameArt kind="component" component={good} label={good} />{/each}</span>
    </span>
  {/if}
{/snippet}

<main data-e2e-layout data-gallery-raw-count={rawGalleryItems.length} data-gallery-composite-count={galleryCategories.find(({ id }) => id === 'demands')?.items.length}>
  <header class="gallery-header" data-e2e-fit data-e2e-no-scroll>
    ...
    <div><p>Production review room</p><h1>Asset gallery</h1><small>73 source assets · 10 composed demand tiles</small></div>
    <p class="manifest-status" role="status" data-status="synced"><i></i>Manifest complete</p>
  </header>
  ...
  <section class="gallery-panel" aria-labelledby="category-title" data-e2e-fit data-e2e-no-scroll>
    ...
    <div class="asset-grid" data-e2e-fit data-e2e-no-scroll>
      {#each visibleItems as item}
        <button class="asset-card" data-asset-id={item.id} aria-label={`Inspect ${item.title}`} onclick={() => selected = item}>
  ...
  {#if selected}
    <div class="review-backdrop" role="presentation" onclick={() => selected = null}>
      <div class="review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-title" tabindex="-1" data-e2e-fit data-e2e-no-scroll onclick={(event) => event.stopPropagation()} onkeydown={(event) => event.stopPropagation()}>
        ...
        <footer><p>{selected.detail}</p><code>{selected.kind === 'demand' ? `composition:${selected.id}` : selected.assetPath}</code></footer>
      </div>
    </div>
  {/if}
</main>
...
<style>
  ...
  .asset-grid { min-height: 0; display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); grid-template-rows: repeat(2, minmax(0, 1fr)); gap: .55rem; overflow: hidden; }.asset-card { min-width: 0; min-height: 0; display: grid; grid-template-columns: minmax(0, 1fr) auto; grid-template-rows: minmax(0, 1fr) auto; gap: .25rem .5rem; overflow: hidden; padding: .38rem; border: 1px solid #d1b574; border-radius: .65rem; color: #173f43; text-align: left; background: #efe0bd; }.asset-card:hover, .asset-card:focus-visible { border-color: #a43b32; outline: 2px solid #a43b32; outline-offset: -2px; }.asset-preview { grid-column: 1 / -1; min-height: 0; display: grid; place-items: center; overflow: hidden; border-radius: .42rem; background: radial-gradient(circle, #fff9e8, #cdb485); }.asset-copy { min-width: 0; display: grid; }.asset-copy strong, .asset-copy small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.asset-copy strong { font-size: .67rem; }.asset-copy small { color: #5f706d; font-size: .5rem; }.asset-card code { align-self: end; color: #995145; font-size: .47rem; white-space: nowrap; }
  ...
  .demand-composition { position: relative; width: min(92%, 14rem); height: 76%; display: grid; place-items: center; overflow: hidden; border: 2px solid #c99b45; border-radius: .5rem; box-shadow: 0 .2rem .5rem #0005; }.demand-composition :global(.demand-base) { position: absolute; inset: 0; width: 100%; height: 100%; filter: brightness(.72); }.demand-composition > span { position: relative; z-index: 1; display: flex; gap: .25rem; }.demand-composition > span :global(.game-art) { width: clamp(1.2rem, 3.5vw, 2.35rem); height: clamp(1.2rem, 3.5vw, 2.35rem); }.demand-composition.large { width: min(96%, 34rem); height: min(75%, 15rem); }.demand-composition.large > span :global(.game-art) { width: clamp(2.7rem, 7vw, 5.5rem); height: clamp(2.7rem, 7vw, 5.5rem); }
