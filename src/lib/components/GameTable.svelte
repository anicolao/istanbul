<script lang="ts">
  ...
  import { defaultMarketSelection } from '$lib/game/market-selection';
  ...
  let previousMarketContext = $state('');
  $effect(() => {
  ...
  const defaultMarketSlots = $derived(defaultMarketSelection(activeDemand?.goods ?? [], localPlayer.goods));
  const flexibleDemandCard = $derived(actionPlace.id === 11
    ? localPlayer.bonusHand.map((cardId) => bonusById.get(cardId)).find((card) => card?.effect === 'wild-small-market')
    : undefined);
  const showFlexibleDemandOffer = $derived(Boolean(
    localIsCurrent && !tabletopControls && !flexibleMarketActive && flexibleDemandCard && defaultMarketSlots.length < (activeDemand?.goods.length ?? 0)
  ));
  const marketContext = $derived((actionPlace.id === 10 || actionPlace.id === 11) && (game.phase === 'action' || game.phase === 'family-action')
    ? `${game.epoch}:${game.turnNumber}:${actionPlace.id}:${activeDemand?.id ?? ''}:${flexibleMarketActive}`
    : '');
  ...
  $effect(() => {
    if (marketContext && marketContext !== previousMarketContext) {
      marketSelection = defaultMarketSlots;
      flexibleGoods = [...(activeDemand?.goods ?? [])];
    }
    previousMarketContext = marketContext;
  });
    ...
    <aside class:finish={game.phase === 'game-over'} class:decision={game.phase !== 'movement'} class="inspector" class:route-planner={!selectedBonusManifest && game.phase === 'movement' && !selectedPlaceManifest} aria-live="polite" data-e2e-fit data-e2e-no-scroll>
      ...
      {:else if game.phase === 'action' || game.phase === 'family-action'}
        ...
        {:else if actionPlace.id === 10 || actionPlace.id === 11}
          <p class="market-instructions">Select one to five depicted goods you own. {actionPlace.id === 10 ? 'Large Market revenue rises from 3 to 25 Lira.' : 'Small Market revenue rises from 2 to 20 Lira.'} Then this Demand rotates.</p>
          ...
          {#if showFlexibleDemandOffer && flexibleDemandCard}
            <aside class="flexible-demand-offer" aria-label="Flexible Demand can fill a missing market good">
              <BonusCard card={flexibleDemandCard} compact mini class="flexible-demand-card" testId="market-flexible-demand-card" />
              <span><strong>Missing a depicted good?</strong><small>Use this card to substitute goods in any Small Market slot.</small></span>
              <button class="turn-action" onclick={() => onPlayBonus(flexibleDemandCard.id, { kind: 'wild-small-market' })}>Play Flexible demand</button>
            </aside>
          {/if}
...
<style>
  ...
  @media (max-width: 720px) {
    ...
    .inspector.decision:has(.demand-card) > .market-instructions, .inspector.decision:has(.demand-card) > .inspector-glyph { display: none; }.inspector.decision:has(.demand-card) > .section-kicker, .inspector.decision:has(.demand-card) > h2 { grid-column: 1 / -1; }.inspector.decision:has(.demand-card) .demand-card { margin-top: .08rem; }
    .inspector.decision:has(.flexible-demand-offer) > .section-kicker { display: none; }.inspector.decision:has(.flexible-demand-offer) > h2 { font-size: 1.1rem; }.inspector.decision:has(.flexible-demand-offer) .demand-card { min-height: 3.25rem; padding-block: .3rem; }.inspector.decision .flexible-demand-offer { grid-template-columns: 2.65rem minmax(0, 1fr); gap: .15rem .4rem; padding: .3rem; }.inspector.decision .flexible-demand-offer :global(.flexible-demand-card) { width: 2.65rem; height: 5.3rem !important; }.inspector.decision .flexible-demand-offer span { padding-left: 0; }.inspector.decision .flexible-demand-offer strong { font-size: .8rem; }.inspector.decision .flexible-demand-offer small { font-size: .5rem; }.inspector.decision .flexible-demand-offer .turn-action { min-height: 1.8rem; padding: .15rem; }
