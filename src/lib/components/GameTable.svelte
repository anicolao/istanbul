        ...
        {:else if actionPlace.id === 14 || actionPlace.id === 15}
          ...
          <div class="mosque-offers" aria-label={`${actionPlace.name} tile offers`}>
            {#each colors as color}
              ...
              <article class={`${color}-tile`}><GameArt kind="component" component={`mosque-${color}` as `mosque-${Good}`} class="mosque-tile-art" /><span>{mosqueColorNames[color]}</span>{#if tile}<strong>{tile.required} {goodNames[color]}</strong><small>{tile.ability}</small>{:else}<strong>Stack empty</strong>{/if}{#if ownsMosqueAbility(currentPlayer, color)}<b>Owned</b>{:else if tile && localIsCurrent}<button class="turn-action" disabled={localPlayer.goods[color] < tile.required} onclick={() => onTakeAction({ kind: 'mosque-take', tileId: tile.id })}>Pay 1 {color} for {mosqueColorNames[color]} tile</button>{/if}</article>
            {/each}
          </div>
  ...
  <section class:hidden-at-finish={game.phase === 'game-over'} class:many={game.players.length > 3} class="player-rail" aria-label="Player resources" style={`--players: ${game.players.length}`}>
    {#each game.players as player, index}
      <article class:local={player.uid === userUid} aria-label={`${player.name} resources`} data-player-color={player.color}>
        ...
        <div class="player-name">{#if index === game.startingSeat}<GameArt kind="piece" piece="first-player" class="first-player-art" />{:else}<span class={`player-dot ${player.color}`}></span>{/if}<strong>{player.name}{player.uid === userUid ? ' · you' : ''}</strong><small>{index === game.startingSeat ? 'Start player' : `Seat ${index + 1}`}</small></div>
        <dl class="resources"><div><GameArt kind="component" component="lira" class="resource-art" /><dt>Lira</dt><dd>{player.lira}</dd></div><div><GameArt kind="component" component="ruby" class="resource-art" /><dt>Rubies</dt><dd>{player.rubies}</dd></div><div><GameArt kind="component" component="wheelbarrow" class="resource-art" /><dt>Capacity</dt><dd>{player.capacity}</dd></div><div><GameArt kind="piece" piece="assistant" color={player.color as PlayerColorName} class="resource-art" /><dt>Assistants</dt><dd>{player.assistantsCarried}</dd></div></dl>
        <div class="goods" aria-label={`${player.name} goods`}>{#each Object.entries(player.goods) as [good, count]}<span class={`good ${good}`} title={goodNames[good as Good]}><GameArt kind="component" component={good as Good} class="good-art" />{count}</span>{/each}</div>
        ...
        {:else}<p class="masked-hand"><GameArt kind="card-back" class="masked-card-art" />Bonus hand · {player.bonusHand.length} hidden card{player.bonusHand.length === 1 ? '' : 's'}</p>{/if}
      </article>
    {/each}
  </section>
</section>
...
<style>
  ...
  .mosque-offers { display: grid; grid-template-columns: 1fr 1fr; gap: .5rem; margin-top: .65rem; }.mosque-offers article { min-width: 0; display: grid; grid-template-columns: 2.7rem 1fr; gap: .25rem .35rem; padding: .55rem; border: 2px solid currentColor; border-radius: .55rem; background: #f1e5cc; }:global(.mosque-tile-art) { grid-row: 1 / 4; width: 2.7rem; height: 2.7rem; }.mosque-offers article > span { font-size: .58rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; }.mosque-offers article > strong { font: 700 1.05rem 'Cormorant Garamond', serif; }.mosque-offers article > small { min-height: 2.4rem; color: #526b68; font-size: .57rem; }.mosque-offers article > b, .mosque-offers article > button { grid-column: 1 / -1; }.mosque-offers article > b { padding: .45rem; border-radius: .35rem; color: #fffaf0; text-align: center; background: currentColor; font-size: .62rem; }.mosque-offers .turn-action { margin-top: auto; font-size: .6rem; }
  ...
  .player-rail article { position: relative; min-width: 0; display: grid; grid-template-columns: auto 1fr auto; gap: .35rem .7rem; overflow: hidden; padding: .55rem .7rem; border: 1px solid rgb(239 202 125 / 55%); border-radius: .85rem; color: #fffaf0; background: #302015; text-shadow: 0 1px 2px #000; }.player-rail article.local { outline: 2px solid #e7b64c; }:global(.player-mat-art) { position: absolute; z-index: 0; inset: 0; opacity: .68; filter: saturate(.9) brightness(.78); }.player-rail article > :not(:global(.player-mat-art)) { position: relative; z-index: 1; }
  .player-name { display: grid; grid-template-columns: 1.55rem auto; align-items: center; }.player-name .player-dot, :global(.first-player-art) { grid-row: 1 / 3; width: 1.4rem; height: 1.4rem; margin-right: .35rem; }:global(.first-player-art) { filter: drop-shadow(0 1px 1px #000); }.player-name small { color: #eadfc9; font-size: .6rem; }
  .resources { display: flex; margin: 0; }.resources div { position: relative; min-width: 2.8rem; padding: 0 .45rem; border-left: 1px solid rgb(239 202 125 / 45%); text-align: center; }.resources dt { color: #eadfc9; font-size: .52rem; text-transform: uppercase; }.resources dd { margin: 0; font-weight: 700; }:global(.resource-art) { width: 1.35rem; height: 1.35rem; float: left; margin-right: .18rem; filter: drop-shadow(0 1px 1px #000); }
  ...
  .goods { display: flex; gap: .25rem; align-items: center; }.good { display: flex; gap: .08rem; align-items: center; font-size: .65rem; font-weight: 700; }:global(.good-art) { width: 1.45rem; height: 1.45rem; filter: drop-shadow(0 1px 1px #000); }
  ...
  .hand { grid-column: 1 / -1; display: flex; gap: .4rem; align-items: center; border-top: 1px solid rgb(239 202 125 / 45%); padding-top: .35rem; font-size: .62rem; }.hand > span { color: #eadfc9; text-transform: uppercase; }.hand button { position: relative; max-width: 12rem; min-height: 2.7rem; display: grid; grid-template-columns: 2rem 1fr; grid-template-rows: auto auto; column-gap: .35rem; overflow: hidden; padding: .2rem .5rem .2rem .2rem; border: 1px solid #c98948; border-radius: .35rem; color: #fffaf0; text-align: left; background: #173f43; }:global(.hand-card-art) { grid-row: 1 / 3; width: 2rem; height: 2.35rem; border-radius: .2rem; }.hand button[aria-pressed='true'] { outline: 2px solid #e7b64c; }.hand button small { font-size: .48rem; text-transform: uppercase; }.hand button strong { overflow: hidden; font-size: .62rem; text-overflow: ellipsis; white-space: nowrap; }
  .masked-hand { grid-column: 1 / -1; min-height: 2.35rem; display: flex; gap: .45rem; align-items: center; margin: 0; padding-top: .35rem; border-top: 1px solid rgb(239 202 125 / 45%); color: #eadfc9; font-size: .62rem; }:global(.masked-card-art) { width: 1.45rem; height: 2.05rem; border-radius: .15rem; box-shadow: 0 .1rem .2rem #0008; }
  ...
  @media (min-width: 721px) and (max-width: 960px) {
    .player-rail article { grid-template-columns: minmax(0, 1fr) auto; }
    .player-rail .resources { grid-column: 1 / -1; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); }
    .player-rail .resources div { min-width: 0; }
  }
  @media (max-width: 720px) {
    ...
    .player-rail { grid-auto-flow: row; grid-template-columns: 1fr 1fr; gap: .3rem; }.player-rail article { padding: .35rem .45rem; grid-template-columns: minmax(0, 1fr) auto; gap: .2rem; }.resources { display: none; }.goods { justify-content: end; }.hand, .masked-hand { padding-top: .2rem; }.hand { min-width: 0; flex-wrap: wrap; }.hand > span { width: 100%; }.hand button { width: 100%; max-width: 100%; min-height: 1.55rem; }.hand:has(button:nth-of-type(3)) { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: .15rem; }.hand:has(button:nth-of-type(3)) > span { grid-column: 1 / -1; }.hand:has(button:nth-of-type(3)) button { min-height: 1.45rem; display: block; padding: .08rem; }.hand:has(button:nth-of-type(3)) :global(.hand-card-art) { width: 100%; height: 1.3rem; }.hand:has(button:nth-of-type(3)) button small, .hand:has(button:nth-of-type(3)) button strong { display: none; }.player-name { min-width: 0; font-size: .72rem; }
