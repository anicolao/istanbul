...
<span class={`location-state state-${placeId}`} data-testid={`place-state-${placeId}`} data-state-summary={summary} data-e2e-status-area>
  ...
  {:else if placeId === 12}
    <span class:empty={policeFamilies.length === 0} class="family-corral" aria-label={`${policeFamilies.length} family members available at Police Station`}>
      {#each policeFamilies as player}<GameArt kind="piece" piece="family" color={player.color} class="station-family" label={`${player.name}'s family available at Police Station`} />{/each}
    </span>
  {:else if placeId === 13}
...
<style>
  ...
  .location-state :global(.game-art) { flex: 0 0 auto; filter: drop-shadow(0 2px 2px #000b); }
  ...
  .family-corral { width: min(100%, 9rem); height: 3.1rem; display: flex; gap: .16rem; align-items: center; justify-content: center; padding: .28rem .42rem; border: 2px solid #cda65f; border-radius: 1.2rem; background: #34251be8; box-shadow: inset 0 0 .45rem #000, 0 2px 3px #0008; }.family-corral.empty::after { color: #d8c294; font-size: .55rem; letter-spacing: .08em; content: 'ON PATROL'; }.family-corral :global(.station-family) { width: min(1.7rem, 18%); height: auto; aspect-ratio: 1; filter: drop-shadow(0 2px 2px #000b); }
  ...
  @media (max-width: 960px) {
    ...
    .family-corral { width: 100%; height: 1.4rem; gap: .04rem; padding: .08rem .15rem; border-width: 1px; border-radius: .55rem; }.family-corral.empty::after { font-size: .24rem; }.family-corral :global(.station-family) { width: min(.78rem, 18%); height: min(.78rem, 88%); }
