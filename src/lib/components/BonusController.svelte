<script lang="ts">
  ...
  const undoText = $derived(!undo ? 'Nothing to undo' : undo.blockedReason ? `Undo locked · ${undo.blockedReason}` : undo.actorUids.includes(userUid) ? `Undo ${undo.label}` : 'Waiting for the tabletop action');
  const undoButtonText = $derived(undo?.blockedReason ? '↶ Locked' : '↶ Undo');
  const privateStatus = $derived(undo?.blockedReason ? `Undo locked · ${undo.blockedReason}` : undo?.actorUids.includes(userUid) ? `Can undo · ${undo.label}` : undoLog.length ? `Last log: Undid ${undoLog.at(-1)?.label}` : isCurrent ? `${game.phase.replace('-', ' ')} · make public choices on the tabletop` : `${currentPlayer.name} is using the tabletop`);
...
<section class="private-controller" aria-labelledby="private-title" data-e2e-fit data-e2e-no-scroll>
  <header>
    <div><p>Private phone · {player.name}</p><span class="private-history"><button class="private-undo" aria-label={undoText} disabled={!canUndo} onclick={onUndo}>{undoButtonText}</button><GameLog entries={gameLog} players={game.players} {userUid} pending={undoPending} {onRollback} /></span></div>
  ...
  <div class="private-workspace" data-e2e-fit data-e2e-no-scroll>
    <nav class="private-hand" aria-label="Private Bonus hand" data-e2e-no-scroll>
      {#each player.bonusHand as cardId}
        ...
        <button aria-label={`Inspect Bonus card: ${card.title}`} aria-pressed={selectedBonus === cardId} onclick={() => onInspectBonus(cardId)}>
          <BonusCard {card} compact mini class="hand-art" /><span>Bonus</span><strong>{card.title}</strong>
        </button>
    ...
    <article class="private-decision" data-e2e-no-scroll>
      {#if atCaravansary}
        ...
        {#if caravanRevealed}
          <div class="revealed" aria-label="Revealed chosen cards">{#each caravanPreview as cardId}<span><BonusCard card={bonusById.get(cardId)!} compact mini /><strong>{bonusById.get(cardId)?.title}</strong></span>{/each}</div>
      ...
      {:else if selected}
        <p class="kicker">Selected Bonus card</p><h2>{selected.title}</h2><BonusCard card={selected} class="large-card" />
        {#if isCurrent}
...
<style>
  ...
  header { padding: .7rem 1rem; border: 1px solid #efca7d66; border-radius: 1rem; background: linear-gradient(110deg, #11383b, #205954); } header > div { display: flex; align-items: center; justify-content: space-between; gap: .5rem; } header p, header > span { margin: 0; color: #efca7d; font-size: .64rem; } header p { font-weight: 700; letter-spacing: .12em; text-transform: uppercase; } h1 { margin: .1rem 0; font: 700 2rem/.95 'Cormorant Garamond', serif; }.private-history { display: flex; gap: .3rem; }.private-undo { min-height: 1.75rem; padding: .2rem .55rem; border: 1px solid #efca7d; border-radius: .4rem; color: #173f43; background: #efca7d; font-size: .58rem; font-weight: 700; white-space: nowrap; }.private-undo:disabled { border-color: #ffffff33; color: #a6bab5; background: #ffffff0d; }
  .private-workspace { min-height: 0; flex: 1; display: grid; grid-template-rows: auto minmax(0, 1fr); gap: .55rem; }
  .private-hand { min-height: 5.1rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(7rem, 1fr)); gap: .45rem; padding: .45rem; overflow: hidden; border: 1px solid #c98948; border-radius: .75rem; background: #24170f; }.private-hand button { min-width: 0; display: grid; grid-template-columns: 2rem 1fr; grid-template-rows: auto 1fr; gap: .15rem .4rem; align-items: center; overflow: hidden; padding: .3rem; border: 1px solid #c98948; border-radius: .5rem; color: #fffaf0; text-align: left; background: #173f43; }.private-hand button[aria-pressed='true'] { outline: 2px solid #efca7d; }.private-hand :global(.hand-art) { grid-row: 1 / 3; width: 2rem; border-radius: .25rem; }.private-hand span { color: #efca7d; font-size: .5rem; text-transform: uppercase; }.private-hand strong { overflow: hidden; font-size: .67rem; text-overflow: ellipsis; }.private-hand p { margin: auto; color: #cabda8; font-size: .75rem; }
  .private-decision { min-height: 0; overflow: hidden; padding: .8rem; border-radius: .9rem; color: #173f43; background: #fffaf0; }.kicker { margin: 0; color: #a43b32; font-size: .6rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; }.private-decision h2 { margin: .2rem 0 .5rem; font: 700 1.55rem 'Cormorant Garamond', serif; }.private-decision > p:not(.kicker) { font-size: .7rem; line-height: 1.3; }.private-decision :global(.large-card) { width: min(100%, 8rem); margin-inline: auto; }
  label, fieldset { display: grid; gap: .25rem; margin-top: .5rem; font-size: .65rem; font-weight: 700; } select { width: 100%; min-height: 2.5rem; padding: .4rem; border: 1px solid #b99a6b; border-radius: .4rem; background: #fff; }.primary { width: 100%; min-height: 2.8rem; margin-top: .55rem; border: 0; border-radius: .5rem; color: #fff; background: #267356; font-weight: 700; }.primary:disabled { opacity: .4; }.caravan-sources { display: grid; grid-template-columns: 1fr 1fr; gap: .4rem; }.revealed { display: grid; grid-template-columns: 1fr 1fr; gap: .4rem; margin-top: .5rem; }.revealed span { min-width: 0; display: grid; grid-template-columns: 1.5rem 1fr; gap: .35rem; align-items: center; padding: .3rem; border: 1px solid #d4bd91; border-radius: .4rem; }.revealed :global(.bonus-card) { width: 1.5rem; }.revealed strong { overflow: hidden; font-size: .6rem; }.card-payments { display: grid; gap: .35rem; }.waiting { color: #6d7c79; }
  @media (max-height: 700px) { h1 { font-size: 1.55rem; }.private-hand { min-height: 4rem; }.private-hand :global(.hand-art) { width: 1.5rem; }.private-decision { padding: .55rem; }.private-decision :global(.large-card) { width: min(100%, 5.5rem); } }
</style>
