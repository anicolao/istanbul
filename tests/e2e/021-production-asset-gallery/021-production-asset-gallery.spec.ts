...
test('the production gallery renders the same logical components used during play', async ({ page }, testInfo) => {
  ...
  await reviewer.step('reviewer-opens-gallery', {
    ...
    verifications: [
      { spec: 'The gallery declares 83 rendered review states backed by the complete 73-asset manifest', check: async () => { const gallery = page.locator('[data-e2e-layout]'); await expect(gallery).toHaveAttribute('data-gallery-rendered-count', '83'); await expect(gallery).toHaveAttribute('data-gallery-raw-count', '73'); await expect(gallery).toHaveAttribute('data-gallery-composite-count', '10'); } },
      { spec: 'All six logical-component category totals are immediately visible', check: async () => { const nav = page.getByRole('navigation', { name: 'Asset categories' }); for (const [name, count] of [['Locations', 16], ['Player trays', 5], ['Playing pieces', 20], ['Bonus cards', 12], ['Market demands', 10], ['Physical components', 20]] as const) await expect(nav.getByRole('button', { name: new RegExp(`${name}.*${count}`) })).toBeVisible(); } },
  ...
  await chooseCategory('Market demands', 'reviewer-opens-large-demand-tiles', 'The reviewer opens all Large MarketDemand states and the first Small Market state', ['demand-large-1', 'demand-large-2', 'demand-large-3', 'demand-large-4', 'demand-large-5', 'demand-small-1']);
